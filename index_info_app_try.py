...
if(clicked1 == True):
    ...
    for info_frame in result.to_dict()["hits"]["hits"]:
        #st.write(info_frame)
        st.write(info_frame['_source']['card_name'])
        for i in info_frame['_source']['category']:
            st.write(i['class'])
            st.write(i['benefit'])
            soup = BeautifulSoup(i['condition'], 'html.parser')
            cleaned_text = soup.get_text()
            st.write(cleaned_text)
    '''
        # https://woori-fisa-bucket.s3.ap-northeast-2.amazonaws.com/index_img/bcc_001.png
